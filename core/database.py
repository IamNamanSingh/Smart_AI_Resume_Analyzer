...
def get_database_connection():
    """Create and return a database connection using DB_PATH"""
    conn = sqlite3.connect(DB_PATH)
    return conn

def init_database():
    """Initialize all tables in a single unified database"""
    ...
    # 7. Create unified feedback table
    cursor.execute('''
...
def save_resume_data(data):
    """Save resume data to database using parameterized query"""
    conn = get_database_connection()
    cursor = conn.cursor()
...
def save_analysis_data(resume_id, analysis):
    """Save resume analysis data using parameterized query"""
    conn = get_database_connection()
    cursor = conn.cursor()
...
def get_resume_stats():
    ...
    try:
        ...
        avg_ats_score = cursor.fetchone()[0] or 0.0
...
def verify_admin(email, password):
    ...
    try:
        admin_email = os.getenv("ADMIN_EMAIL", "admin@smartresume.ai").strip()
        admin_password = os.getenv("ADMIN_PASSWORD", "admin_password_123").strip()
        return email.strip() == admin_email and password.strip() == admin_password
    except Exception as e:
...
def get_detailed_ai_analysis_stats():
    ...
    try:
        ...
        print(f"Error getting detailed AI stats: {e}")
        return {
            "total_analyses": 0,
            "model_usage": [],
            "average_score": 0.0,
